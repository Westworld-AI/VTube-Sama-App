...
from utils.gui_utils import get_screen_size, get_active_window, get_named_windows, get_named_windows_fallback
...
class Config(metaclass=Singleton):
    ...
    VideoFrameExtractor_path = "./res/tool/subfinder/VideoSubFinder/VideoSubFinderWXW"
    ...
    def set_fixed_seed(self, is_fixed: bool = True, seed: int = DEFAULT_FIXED_SEED_VALUE,
                       temperature: float = DEFAULT_FIXED_TEMPERATURE_VALUE) -> None:
    ...
    def _set_env_window_info(self):
        ...
        if len(named_windows) == 0 or len(named_windows) > 1:
            self._config_warn(f'-----------------------------------------------------------------')
            self._config_warn(
                f'Cannot find unique env window nor pattern: {self.env_name}|{self.win_name_pattern}. Assuming this is an offline test run!')
            self._config_warn(f'-----------------------------------------------------------------')
        else:
            ...
            assert self._min_resolution_check(
                env_window), 'The resolution of env window should at least be 1920 X 1080.'
    ...
    def _min_resolution_check(self, env_window):
        return env_window.width >= self.DEFAULT_ENV_RESOLUTION[0] and env_window.height >= self.DEFAULT_ENV_RESOLUTION[
            1]

    def _aspect_ration_check(self, env_window):
        return env_window.width * self.DEFAULT_ENV_SCREEN_RATIO[1] == env_window.height * self.DEFAULT_ENV_SCREEN_RATIO[
            0]
