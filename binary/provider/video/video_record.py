...
from provider.video.video_easyocr_extract import VideoEasyOCRExtractProvider

config = Config()
logger = Logger()
video_ocr_extract = VideoEasyOCRExtractProvider()
...
class VideoRecordProvider:
    ...
    def __init__(self,
        ...
        self.screen_shot_dir = os.path.join(os.path.dirname(self.video_path), 'screen_shot')
        ...
        while True:
            ...
            if success:
                ...
                if frame_count == total_frames:
                    ...
                    # 格式化文件名，包含时间戳和随机数
                    file_name = f"screen_{timestamp}_{random_number}.jpg"
                    screen_shot_image_path = os.path.join(self.screen_shot_dir, file_name)
                    # 保存最后一帧
                    cv2.imwrite(screen_shot_image_path, frame)
                    print(f"提取完成，已保存最后一帧：{file_name}。 image_path:{screen_shot_image_path}")
                    break
            else:
                break
        cap.release()
        return screen_shot_image_path
    ...
    def capture_screen(self, frame_buffer: FrameBuffer):
        ...
        video_slice_path = os.path.join(self.video_path_dir, video_name + '_slice_{:06d}.mp4'.format(
            self.frames_count // self.frames_per_slice))
            ...
            while self.thread_flag:
                try:
                    ...
                    if config.ocr_enabled:
                        cur_text = video_ocr_extract.extract_text(frame, return_full=0)
                        cur_text = cur_text[0]
